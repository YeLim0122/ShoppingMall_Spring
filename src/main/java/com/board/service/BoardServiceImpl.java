...
public class BoardServiceImpl implements BoardService {
	...
	public int rewriteBoard(BoardVO board) {
		// [1] 부모글(원글)의 글번호(num)로 부모글의 refer(글 그룹 번호), lev(답변 레벨), 
		//		sunbun(순번) 가져오기
			// ==> select 문 
		BoardVO parent = this.selectRefLevSunbun(board.getNum());
		  
		// [2] 기존에 달린 답변글들이 있다면 내 답변글을 insert하기 전에 
		//		기존의 답변글들의 sunbun을 하나씩 증가시키기
			// ==> update 문
		...
		board.setRefer(parent.getRefer());	// 글 그룹 번호를 부모와 동일하게
		board.setLev(parent.getLev()+1);	// 답변레벨 = 부모 lev+1
