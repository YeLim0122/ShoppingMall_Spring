...
public class ShopServiceImpl implements ShopService {
	...
	public int addCart(CartVO cartVo) {
		...
		// 1) 회원번호와 상품번호로 cart 테이블에 있는 상품 개수 가져오기
		Integer cnt = cartMapper.selectCartCountByPnum(cartVo);
		
		// 1_1) 장바구니에 추가할 상품이 이미 장바구니에 담겨있다면 => 수량만 수정 - UPDATE
		if (cnt != null) {
			int n = cartMapper.updateCartQty(cartVo);
			return n; 
		} else {	
			// 1_2) 장바구니에 없는 상품을 추가한다면 => INSERT
	...
	public int editCart(CartVO cartVo) {
		...
		if (qty == 0) {	// 수량이 0이면 삭제 처리
			...
			throw new NumberFormatException("잘못된 값입니다.");
		} else if (qty > 50) {
			throw new NumberFormatException("수량은 50개 이하로만 수정 가능합니다.");
		} else {
