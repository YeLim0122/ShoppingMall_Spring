...
// 서비스 계층: @Service
// 영속성 계층: @Repository
// 주의) servlet-context.xml에 component-scan 대상으로 패키지 등록해야 함.
...
public class MemoDAOMyBatis implements MemoDAO {
	...
	// @Resource: 리소스 이름으로 주입한다. id가 sqlSessionTemplate인 객체를 찾아서 주입      
	...
	public int deleteMemo(int no) {
		...
		// delete()메소드 호출 => MemoMapper.xml에서 <delete>delete문 완성 </delete>
	...
	public MemoVO getMemo(int no) {
		...
		MemoVO memo = session.selectOne(NS+".getMemo", no);
